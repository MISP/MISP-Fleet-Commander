...
def upgrade():
    ...
    sa.Column('setting_name', sa.String(length=120), nullable=False),
    sa.Column('setting_value', sa.String(), nullable=True),
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], ),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('setting_name', 'setting_value')
    )
    with op.batch_alter_table('user_settings', schema=None) as batch_op:
        batch_op.create_index(batch_op.f('ix_user_settings_setting_name'), ['setting_name'], unique=False)
...
def downgrade():
    ...
    with op.batch_alter_table('user_settings', schema=None) as batch_op:
        ...
        batch_op.drop_index(batch_op.f('ix_user_settings_setting_name'))
